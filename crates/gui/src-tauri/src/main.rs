...
/// Generated by `icons/render-icons.mjs` from the same swept arc as the app
/// icon, so the menu bar and the Dock agree on what this app's symbol is. It
/// used to be the sidebar's Cleanup glyph, borrowed — which meant the menu bar
/// wore one module's icon as though it were the whole app's.
